...
public class JpaPermissionRepositoryAdapter implements PermissionRepositoryPort {
    private final SpringDataPermissionRepository repo;
    private final PermissionPersistenceMapper mapper;

    public JpaPermissionRepositoryAdapter(SpringDataPermissionRepository repo, PermissionPersistenceMapper mapper) {
        this.repo = repo;
        this.mapper = mapper;
    }

    public List<Permission> findAllByNames(Collection<String> names) {
        return names == null || names.isEmpty() ? List.of()
                : repo.findAllByNameIn(names).stream().map(mapper::toDomain).toList();
    }
}
