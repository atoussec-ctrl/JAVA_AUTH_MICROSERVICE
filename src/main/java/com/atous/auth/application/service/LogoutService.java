...
import com.atous.auth.application.port.in.LogoutUseCase;
...
public final class LogoutService implements LogoutUseCase {
    ...
    public LogoutService(RefreshTokenRepositoryPort tokens, TokenHashingPort hash, ClockProviderPort clock) {
        this.tokens = tokens;
        this.hash = hash;
        this.clock = clock;
    }

    public LogoutResult execute(LogoutCommand c) {
        tokens.revoke(hash.hash(c.refreshToken()));
        return new LogoutResult(true, "Logout completed successfully", clock.now());
    }
}
